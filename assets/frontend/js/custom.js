...
$(document).ready(function () {
	...
	// Countdown //
	$('#countdown').countdown({
		until: new Date(2015, 5 - 1, 28), // new Date(year, mth - 1, day, hr, min, sec) - date/time to count down to 
	...
	// Google Map //
	$('#map_canvas').gmap({
		'center': new google.maps.LatLng(-6.362353, 106.824016), // Change this to your desired latitude and longitude
		'zoom': 17,
	...
	var image = {
		url: host+path+'/assets/frontend/images/marker.png', // Define the map marker file here
	...
	$('#map_canvas').gmap().bind('init', function () {
		$('#map_canvas').gmap('addMarker', {
			'id': 'marker-1',
			'position': '-6.362353,106.824016',
			'bounds': false,
			'icon': image
		}).click(function () {
			$('#map_canvas').gmap('openInfoWindow', {
				'content': '<h4>CCIT EXPO 2015</h4><p><strong>Auditorium FTUI</strong><br>Universitas Indonesia</p>'
			}, this);
		});
	});
